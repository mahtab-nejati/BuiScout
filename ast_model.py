...
class AST(nx.DiGraph):
    ...
    def set_nodes_levels(self, *args, **kwargs):
        ...
        current_level_nodes = dict(
            filter(lambda node: node[-1]["type"] == ls.ROOT_TYPE, self.nodes.items())
        )
    ...
    def get_affected_nodes(self, *args, **kwargs):
        ...
        ignored_nodes = filter(
            lambda node_data: node_data["type"] in ls.IGNORED_TYPES,
...
class ASTDiff(object):
    ...
    def __init__(
        self, source, destination, matches, file_name, commit_hash, *args, **kwargs
    ):
        self.file_name = file_name
        self.commit_hash = commit_hash
        self.source = AST(source, file_name=file_name, commit_hash=commit_hash)
        self.destination = AST(
            destination, file_name=file_name, commit_hash=commit_hash
        )
    ...
    def summarize(self, method="SUBTREE", *args, **kwargs):
        ...
        for level in range(depth):
            ...
            in_process = filter(
                ...
                and node_data["type"] not in ls.IGNORED_TYPES,
        ...
        self.summary[method].append(
            {
                ...
                "destination_node_summary": ls.stringify(
